...
for(i in 1:ncol(ec.eqv)){
  ec.stdil[,i] <- ifelse(smp.tp==levels(smp.tp)[2],
    ...
    ifelse(smp.tp==levels(smp.tp)[6], 
      ifelse(foods$type=="prepared food" | is.na(foods$type) | is.na(foods$wght),500, foods$wght[]*2),
    ifelse(smp.tp==levels(smp.tp)[9], 
    ifelse(tdata>=refdate, as.numeric(as.character(swabs$area[]))/6, as.numeric(as.character(swabs$area[]))/8),1)))));
}
